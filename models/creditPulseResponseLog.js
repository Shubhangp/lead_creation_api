...
const TABLE_NAME = 'credit_pulse_response_logs';

class CreditPulseResponseLog {
    ...
    static _calculateStats(items, source, startDate, endDate) {
        const stats = {
            ...
            sourceBreakdown: {},
            statusCategoryBreakdown: { Success: 0, Fail: 0, null: 0, other: 0 },
            sourceWiseStats: {},
            successRateBySource: {},
            successRate: '0%'
        };
        ...
        items.forEach(item => {
            ...
            if (!stats.sourceWiseStats[src]) {
                stats.sourceWiseStats[src] = {
                    totalLogs: 0, success: 0, fail: 0, error: 0, null: 0, other: 0
                };
            }
            ...
            } else if (actualStatus === 'failure' || actualStatus === 'fail') {
    ...
    // ─── Helper: Extract status from responseBody ──────────────────────────────
    ...
    static _extractStatus(item) {
        ...
        try {
            ...
            // CreditPulse response shape: { status: "success" | "failure", message: "..." }
    ...
    static async getStatsByDate(sourceOrStartDate, startDateOrEndDate, endDate) {
        ...
        try {
            ...
            // responseStatus + responseBody) — skips large fields like requestPayload
            // so high-volume days (100k–200k+ rows) stay fast and don't exhaust memory.
        ...
        items.forEach(item => {
            ...
            if (!statsByDate[date]) {
                statsByDate[date] = {
                    ...
                    total: 0,
                    statusBreakdown: {},
                    statusCategories: { Success: 0, Fail: 0, null: 0, other: 0 },
                    sourceBreakdown: {},
                    bySource: {}
                };
            }
            ...
            if (!statsByDate[date].bySource[src]) {
                statsByDate[date].bySource[src] = { total: 0, success: 0, fail: 0, error: 0, null: 0, other: 0 };
            }

            statsByDate[date].bySource[src].total++;

            if (actualStatus === 'success') statsByDate[date].bySource[src].success++;
            else if (actualStatus === 'failure' || actualStatus === 'fail') statsByDate[date].bySource[src].fail++;
            else if (actualStatus === 'error') statsByDate[date].bySource[src].error++;
...
module.exports = CreditPulseResponseLog;