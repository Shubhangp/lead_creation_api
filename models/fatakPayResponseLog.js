...
class FatakPayResponseLog {
  ...
  static _groupByDate(items) {
    ...
    items.forEach(item => {
      ...
      const src = item.source || 'unknown';
      if (!map[date]) {
        map[date] = { date, total: 0, statusBreakdown: {}, messageBreakdown: {}, permanentBlocks: 0, eligible: 0, notEligible: 0, leadExists: 0, loanExists: 0, statusCategories: { eligible: 0, notEligible: 0, leadExists: 0, loanExists: 0, other: 0 }, sourceBreakdown: {}, bySource: {} };
      }
      map[date].total++;
      map[date].sourceBreakdown[src] = (map[date].sourceBreakdown[src] || 0) + 1;
      if (!map[date].bySource[src]) {
        map[date].bySource[src] = { total: 0, eligible: 0, notEligible: 0, leadExists: 0, loanExists: 0, other: 0 };
      }
      map[date].bySource[src].total++;
      const st = item.responseStatus || 'unknown';
      map[date].statusBreakdown[st] = (map[date].statusBreakdown[st] || 0) + 1;

      // Assign each record to exactly ONE mutually-exclusive, message-driven
      // category. `other` is only the genuine uncategorized remainder.
      let category = 'other';
      if (item.responseBody) {
        let body = item.responseBody;
        if (typeof body === 'string') { try { body = JSON.parse(body); } catch (_) {} }
      ...
      if (category !== 'other') map[date][category]++;
    });
