...
class CrmPaisaResponseLog {
  ...
  // ✅ REMOVED: findAll() - use findBySource() to avoid scans
  ...
  static async getStats(source = null, startDate = null, endDate = null) {
    ...
    allItems.forEach(item => {
      ...
      // Initialize status categories dynamically
      if (!stats.statusCategoryBreakdown[rs]) {
        stats.statusCategoryBreakdown[rs] = 0;
      }
      stats.statusCategoryBreakdown[rs]++;
      ...
      stats.sourceWiseStats[src].statusBreakdown[rs] =
        (stats.sourceWiseStats[src].statusBreakdown[rs] || 0) + 1;

      // Parse responseBody for messages
    ...
    // Calculate success rates (assuming success = 200/201/success status)
    Object.keys(stats.sourceWiseStats).forEach(src => {
      const s = stats.sourceWiseStats[src];
      const successStatuses = ['200', '201', 'success', 'Success'];
      const successCount = successStatuses.reduce((sum, status) => 
        sum + (s.statusBreakdown[status] || 0), 0);
      s.successRate = s.totalLogs > 0
        ? ((successCount / s.totalLogs) * 100).toFixed(2) + '%' : '0%';
      stats.successRateBySource[src] = s.successRate;
    });

    const successStatuses = ['200', '201', 'success', 'Success'];
    const totalSuccess = successStatuses.reduce((sum, status) => 
      sum + (stats.statusCategoryBreakdown[status] || 0), 0);
    stats.successRate = allItems.length > 0
      ? ((totalSuccess / allItems.length) * 100).toFixed(2) + '%' : '0%';
  ...
  static async getStatsByDate(sourceOrStart, startOrEnd, endDate) {
    ...
    allItems.forEach(item => {
      ...
      if (!map[date].bySource[src]) {
        map[date].bySource[src] = { total: 0, statusBreakdown: {} };
      }
      map[date].bySource[src].total++;
      map[date].bySource[src].statusBreakdown[rs] =
        (map[date].bySource[src].statusBreakdown[rs] || 0) + 1;
    });
...
module.exports = CrmPaisaResponseLog;