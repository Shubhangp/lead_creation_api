...
class Lead {
  ...
  static validate(data) {
    ...
    if (data.creditScore !== undefined && data.creditScore !== null &&
      (data.creditScore < 300 || data.creditScore > 900))
  ...
  /**
   * ✅ BULK-FRIENDLY: Add N to the counter in a single UpdateCommand.
   * Called once per source at the end of a bulk upload instead of N times.
   */
  ...
  static async findByDateRange(startDate, endDate, options = {}) {
    ...
    try {
      ...
      const results = await Promise.all(
        partitions.map(p => this._queryByDatePartition(p, startDate, endDate, limit))
      );
      const allItems = results.flat();
      const items = limit ? allItems.slice(0, limit) : allItems;
      return { items, lastEvaluatedKey: null, count: items.length };
    } catch (error) {
  ...
  static async updateById(leadId, updates) {
    ...
    Object.keys(finalUpdates).forEach((key, index) => {
  ...
  static async getAccurateTotalCount() {
    ...
    try {
      ...
      if (sources.length === 0) return await this._countViaDatePartitions();
  ...
  static async _countViaDatePartitions() {
    ...
    const partitions = this.getMonthPartitions(startDate.toISOString(), endDate.toISOString());
    console.log(`  Counting ${partitions.length} monthly partitions...`);

    const results = await Promise.all(partitions.map(p => this._countFullPartition(p)));
  ...
  static async getQuickStats(source = null, startDate = null, endDate = null) {
    try {
      if (source && !startDate) {
        ...
        return { totalLogs: count, source, isEstimate: false, method: 'atomic-counter' };
  ...
  static async getQuickStatsForDateRange(startDate, endDate) {
    ...
    try {
      ...
      if (sources.length === 0) return this._countDateRangeWithoutSources(startDate, endDate);
  ...
  static async _countDateRangeWithoutSources(startDate, endDate) {
    const partitions = this.getMonthPartitions(startDate, endDate);
    const results = await Promise.all(
      partitions.map(p => this._countDatePartition(p, startDate, endDate))
    );
    return {
      totalLogs: results.reduce((sum, count) => sum + count, 0),
      isEstimate: false,
      method: 'gsi-query',
      dateRange: { start: startDate, end: endDate }
    };
  ...
  static async getStats(startDate, endDate, options = {}) {
    ...
    if (!startDate || !endDate) {
      throw new Error('getStats() requires startDate and endDate parameters.');
    }
    ...
    try {
      ...
      for (let i = 0; i < partitions.length; i++) {
        ...
        await this._streamProcessPartition(partition, startDate, endDate, statsAggregator, (count) => {
          totalProcessed += count;
          if (progressCallback) {
            progressCallback({ partition: i + 1, totalPartitions: partitions.length, processedRecords: totalProcessed });
          }
        });
      }

      const stats = this._finalizeStats(statsAggregator, startDate, endDate, startTime);
      console.log(`[${TABLE_NAME}] ✅ Streaming stats complete: ${stats.totalLogs.toLocaleString()} records in ${Date.now() - startTime}ms`);
      return stats;
    } catch (error) {
  ...
  static _createStatsAggregator() {
    return {
      totalLogs: 0,
      seenPhones: new Set(), seenPans: new Set(),
      duplicatePhones: new Set(), duplicatePans: new Set(),
      sourceBreakdown: {},
      ...
      jobTypeBreakdown: {}, businessTypeBreakdown: {},
  ...
  static _processChunk(items, aggregator) {
    items.forEach(item => {
      ...
      if (item.phone) {
        if (aggregator.seenPhones.has(item.phone)) aggregator.duplicatePhones.add(item.phone);
        aggregator.seenPhones.add(item.phone);
      }
      if (item.panNumber) {
        if (aggregator.seenPans.has(item.panNumber)) aggregator.duplicatePans.add(item.panNumber);
      ...
      if (aggregator.genderBreakdown[gender] !== undefined) aggregator.genderBreakdown[gender]++;
      else aggregator.genderBreakdown['Other']++;

      let age = item.age;
      if (!age && item.dateOfBirth) age = this.calculateAge(item.dateOfBirth);
      aggregator.ageRangeBreakdown[age ? this.getAgeRange(age) : 'Unknown']++;

      if (item.jobType) aggregator.jobTypeBreakdown[item.jobType] = (aggregator.jobTypeBreakdown[item.jobType] || 0) + 1;
      if (item.businessType) aggregator.businessTypeBreakdown[item.businessType] = (aggregator.businessTypeBreakdown[item.businessType] || 0) + 1;

      if (item.salary) {
        const s = parseInt(item.salary);
        if (s < 20000) aggregator.salaryRangeBreakdown['Below 20k']++;
        else if (s < 40000) aggregator.salaryRangeBreakdown['20k-40k']++;
        else if (s < 60000) aggregator.salaryRangeBreakdown['40k-60k']++;
        else if (s < 80000) aggregator.salaryRangeBreakdown['60k-80k']++;
        else if (s < 100000) aggregator.salaryRangeBreakdown['80k-100k']++;
      ...
      aggregator.consentBreakdown[
        item.consent === true ? 'true' : item.consent === false ? 'false' : 'unknown'
      ]++;
    });
  }

  static _finalizeStats(aggregator, startDate, endDate, startTime) {
    let uniqueLeads = 0, duplicateLeads = 0;
    aggregator.seenPhones.forEach(phone => {
      if (aggregator.duplicatePhones.has(phone)) duplicateLeads++;
      else uniqueLeads++;
    });
  ...
  static async getStatsByDate(startDate, endDate) {
    ...
    try {
      ...
      console.log(`[${TABLE_NAME}] Stats by date complete in ${Date.now() - startTime}ms`);
      return result;
    } catch (error) {
    ...
    do {
      ...
      items.forEach(item => {
        ...
        if (!statsByDate[date]) {
          statsByDate[date] = {
            date, total: 0,
            uniquePhones: new Set(), uniquePans: new Set(),
        ...
        const gender = item.gender || 'Unknown';
        if (statsByDate[date].genders[gender] !== undefined) statsByDate[date].genders[gender]++;
        else statsByDate[date].genders['Other']++;
        if (item.consent === true) statsByDate[date].withConsent++;
        else statsByDate[date].withoutConsent++;
      });
  ...
  static async backfillDatePartitions(sourceToMigrate = null) {
    ...
    try {
      const sources = sourceToMigrate
        ? [sourceToMigrate]
        : (process.env.LEAD_SOURCES?.split(',') || []);

      if (sources.length === 0) {
        throw new Error('No sources configured. Set LEAD_SOURCES env var or provide source parameter.');
      }
  ...
  static async _backfillSource(source) {
    ...
    do {
      ...
      const updates = (result.Items || [])
        ...
        .map(item => this.updateByIdNoValidation(item.leadId, {
